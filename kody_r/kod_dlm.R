...
opt <- function(param,yields){
  cost <- tryCatch(expr = {
    ...
    result <- Build_DieboldLi(param = param, yields = yields)
...
library(pso)
aa <- psoptim(par = param0, fn = opt, lower = -10, upper = 10, control = list(trace = 6, s=100, maxit=10000), yields = yields)
results <- optim(par = param0, fn = opt, method = "L-BFGS-B", yields = yields,control = list(trace = 6, maxit=1000))
library(maxLik)
aa <- maxLik(logLik = opt, start = param0, method = "BHHH", control = list(printLevel = 6), yields = yields)
par0 <- aa$par
